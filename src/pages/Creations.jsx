...
const Creations = () => {
  ...
  return (
    <div ref={pageRef}>
      ...
      <SecondaryHero eyebrow="Our Portfolio" title="Precision" highlight="UAV Creations." description="Every component we craft is a testament to our relentless pursuit of excellence. From propulsion to avionics, our creations are designed to perform where it matters most." watermark="UAV" />

      {categories.map(({ num, category, tagline, items, img, watermark }, index) => (
        <section key={category} data-stack-section className={`relative overflow-hidden py-[110px] max-sm:py-[72px] ${index % 2 === 0 ? 'bg-white' : 'bg-[#fff8f1]'}`}>
          <ParallaxWatermark className={`top-1/2 -translate-y-1/2 text-[clamp(6rem,12vw,15rem)] ${index % 2 === 0 ? 'left-0 text-[#ff6b00]/[0.06]' : 'right-0 text-[#111111]/[0.05]'}`} speed={22}>{watermark}</ParallaxWatermark>
          <div className="relative mx-auto grid w-full max-w-[1380px] grid-cols-[minmax(0,0.48fr)_minmax(0,0.52fr)] items-center gap-[clamp(44px,6vw,84px)] px-[clamp(20px,5vw,80px)] max-xl:grid-cols-1">
            <div className={index % 2 === 1 ? 'max-xl:order-2' : ''}>
              <SecondarySectionIntro eyebrow={`${num} ${category}`} title={category} description={tagline} className="max-w-[640px]" />
              <div className="mt-10 grid gap-5">
                {items.map((item) => (
                  <article key={item.name} className="book-reveal-card rounded-[18px] border border-[#f1dfd1] bg-white px-6 py-6 shadow-[0_18px_42px_rgba(17,21,26,0.06)] transition-all duration-300 hover:-translate-y-1 hover:border-[#ff6b00]/45">
                    <h3 className="text-[21px] font-extrabold text-[#111111]">{item.name}</h3>
                    <p className="mt-3 text-[15px] leading-[1.78] text-[#67707d]">{item.desc}</p>
            ...
            <div className={index % 2 === 1 ? 'xl:order-first' : ''}>
              <ImageFrame variant="landscape" src={img} alt={category} width="760" height="560" />
            </div>
          </div>
        </section>
      ))}

      <SecondaryCta title="Need a Custom UAV Solution?" description="Our engineering team specialises in bespoke UAV component design and integration. Let us build your next breakthrough together." primaryLabel="Talk to Our Engineers" primaryTo="/contact" />
    </div>
  );
};
